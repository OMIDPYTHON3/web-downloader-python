...
'max_concurrent':4,'default_threads':8,'smart_download':True,'retry_count':5,'retry_delay':3,'auto_resume':True,}
def load():
    c=DEFAULT.copy()
    ...
    c.pop('global_speed_limit', None)
    c['default_threads']=max(1,min(32,int(c.get('default_threads',8))))
    c['max_concurrent']=max(1,min(8,int(c.get('max_concurrent',4))))
